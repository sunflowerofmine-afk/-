...
# 교집합을 요구하지 않는 관대한 경로(고가수축형·고가횡보형)에서 같은 종목이
# 최근 10거래일 중 이 일수 이상 반복 발화하면 매수검토→관찰 강등. 표본·후보 왜곡 방지.
REPEAT_SUPPRESS_MIN_COUNT                = 2
...
# ── 실제 운용 규칙 (2026-09-08 사용자 확정) ─────────────────────────────
# 성과 지표를 **비율이 아니라 금액**으로도 재기 위한 값.
# 왜 필요한가: `review.is_win`은 익일 시가가 기준가보다 높은지만 본다. -20% 갭과
# -0.1% 갭이 똑같이 "실패 1건"이다. 그런데 사용자의 실제 규칙은 금액 기준이고
# (감당 손절 50만/1회), 그 선을 넘었는지가 매매를 계속할 수 있는지를 가른다.
# 미모사 6강 01:33:04 "감당 가능한 손절 금액을 생각해서 매수 금액을 정한다".
POSITION_KRW_NORMAL   = 10_000_000   # 평소 1종목 비중 (횡보·하락장)
POSITION_KRW_CONVICT  = 15_000_000   # 확신 시 (장세·테마 확실할 때)
PAIN_STOP_KRW         =    500_000   # 1회 감당 손절 — 넘으면 심리적 타격
DAILY_STOP_KRW        =    500_000   # 일일 중단선
CAMPAIGN_STOP_KRW     =  2_000_000   # 운용 중단선
